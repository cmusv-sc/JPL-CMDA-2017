...
// This fundtion implements the Nearest Neighbor Algorithm.
// int search_nn (float *tgt_x, float *tgt_y, float *qry_x, float *qry_y, 
int search_nn (float tgt_x[], float tgt_y[], float qry_x[], float qry_y[], 
               int qry_idx1, int qry_idx2, float qry_range, int cell_search_limit) {
	printf("****** in search_nn");
   /*
   ...
   */
	...
	int n = sizeof(tgt_x)/sizeof(float);
	int n1 = sizeof(tgt_y)/sizeof(float);
	int m = qry_idx2 - qry_idx1 + 1;
	int nqy = sizeof(qry_y)/sizeof(float);
	int nqx = sizeof(qry_x)/sizeof(float);

	if (n != n1 || m > nqy || m > nqx) {
	    printf("****** Error: check the dimensions of your input arrays!");
	    return -1;
	}
