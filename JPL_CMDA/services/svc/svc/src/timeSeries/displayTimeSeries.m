...
for fileI = 1:nFiles
  ...
  if varIs2d
    ...
    thisData = zeros(size(monthIdx1:monthIdx2));
    for pI = 1:nP
      [p_idx, p_alphas] = linearInterpHelper(dataInfo(varI).plev(pI), plev, 'log');
      for pIdx = 1:length(p_idx)
        nLon_tmp = length(lonIdx);
        nLat_tmp = length(latIdx);
        v = ncreadVar(thisFile, varName, [lonIdx(1), latIdx(1), p_idx(pIdx), idx2Data_start], [nLon_tmp, nLat_tmp, 1, idx2Data_stop - idx2Data_start + 1]);
        thisData = thisData + p_alphas(pIdx) * averageOverSphere(reshape(v, nLon_tmp, nLat_tmp, []), lat, dataInfo(varI).latRange);
      end
    end
    monthlyData(monthIdx1:monthIdx2) = thisData/nP;
  end
